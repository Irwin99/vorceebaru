...
class Payment extends CI_Controller {
	...
	public function __construct(){
		parent::__construct();
		$this->load->model('mpayment','mpy');
    	$this->load->library('pagination');
  	}

  function manage_payment(){
    $data['title_web'] = 'View All payment | Adminpanel Vorcee';
    ...
    $data['results'] = $this->mpy->fetchPayment($config['per_page'],$page,$this->uri->segment(4)); // fetch data using limit and pagination
    ...
    $this->load->view('admin/index',$data);
			...
			$data['results'] = $this->mpy->fetchPaymentSearch($_POST); // fetch data using limit and pagination
			$data['links'] = false;
			$this->load->view('admin/index',$data);
		}
  }
	...
	function add_payment(){
		$data['title_web'] = 'Add payment | Adminpanel Vorcee';
		...
		if(!$this->form_validation->run()){
			...
			$save = $this->mpy->savepayment($_POST);
...
function edit_payment(){
		$data['title_web'] = 'Edit payment | Adminpanel Vorcee';
		...
		if($data['result']==FALSE)
			...
			$save = $this->mpy->editpayment($_POST,$id);
			redirect(base_url($this->uri->segment(1).'/payment/manage_payment'));
		}
}
